...
$requete = $conn->prepare("SELECT * FROM `postits`");
$requete->execute();

// On obtient toutes les lignes sous forme d'un tableau stocké dans $postits
$postits = $requete->fetchAll();

//print_r: Utile pour débugger et afficher le contenu d'un tableau ou d'un objet
//json_encode: Pour convertir les données dans le format JSON
//print_r(    json_encode($postits)   );
...
<html lang="fr">
  <head>
    ...
    <script>
      let postIts = <?= json_encode($postits) ?>;
    </script>
  </head>
  <body>
