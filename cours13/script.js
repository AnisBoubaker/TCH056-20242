...
$( document ).ready(function() {
    ...
    document.getElementById('createPostItForm').addEventListener('submit', function(e) {
        ...
        const nouveau = {id: maxId.id+1, title: title, content:content};

        fetch("/api/postits", {method:"POST", body: nouveau})
        .then(response => {
            if(! response.ok){
                throw new Error("Erreur lors de la creation du nouveau postit");
            }
            return response.json();
        })
        .then(data => {
            postIts.push(nouveau)
            renderPostIts();
        })
        .catch(error=>{
    ...
    //Obtention et affichage de la liste des postits
    fetch("/api/postits")
    .then(response => {
        if(! response.ok){
            throw new Error("Erreur lors de la recuperation de la liste des postits");
        }
        return response.json();
    })
    .then(data => {
